...
export default function CompleteSHTRewardsPage() {
  ...
  // 安全获取用户统计数据
  const safeGetUserStat = (index: number, defaultValue: string = '0') => {
    try {
      if (!userStats || !Array.isArray(userStats) || userStats.length <= index) {
        return defaultValue;
      }
      const value = userStats[index];
              ...
              <div className="text-center space-y-3">
                ...
                {totalSupply && (
                  <div className="text-xs text-muted-foreground pt-2 border-t">
                    总供应量: {safeFormatEther(totalSupply)} SHT
                  </div>
                )}
                {lastRefresh && (
              ...
              <div className="space-y-3">
                ...
                <div className="text-center pt-2 border-t">
                  <div className="text-lg font-bold text-blue-700">
                    {userStats && userStats[3] ? safeFormatEther(userStats[3]) : '0'}
                  </div>
